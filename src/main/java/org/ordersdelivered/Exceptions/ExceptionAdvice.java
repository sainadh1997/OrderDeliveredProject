...
public class ExceptionAdvice {
	...
	public String resultNotFoundException(EmptyResultDataAccessException e, Model uiModel) {
		logger.info("error is"+e.getMessage());
		uiModel.addAttribute("exceptionOccurs", "Given Order-Id is not found in database,Please search your Id carefully.");
		return "error";
	}
	 ...
	 @ExceptionHandler(value=DataIntegrityViolationException.class)
		public String pageNotException(DataIntegrityViolationException e, Model uiModel) {
			logger.info("error is"+e.getMessage());
			uiModel.addAttribute("delete", "Cannot delete because You must delete the same Order id in the delivered_Updates table, then you be deleted the same Order id in the Orders table");
			return "error";
		}
